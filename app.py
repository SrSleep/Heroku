...
from flask import Flask, render_template, flash, request, redirect, url_for, jsonify,g, send_file,make_response, session
import utils
...
def register():
    ...
    try:
        if request.method == 'POST':
            ...
            db.executescript(
                "INSERT INTO usuario (nombre, usuario, correo, contraseña) VALUES ('%s','%s','%s','%s')" % (name, username, email, hashPassword)
            )
            db.commit()
    ...
    try:
        if request.method == 'POST':
            ...
            if user is None:
                ...
                storagePassword = user[4]
                resultado = check_password_hash(storagePassword,password)
